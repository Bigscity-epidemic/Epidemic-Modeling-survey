...
settings = json.load(open('settings-shh-closure.json', encoding='utf8'))
r0 = settings['r0']
...
for index in range(future):
    ...
    if settings['全员核酸周期'] == 0:
        ...
        if index % settings['全员核酸周期'] == 0 and index != 0 and index > 30:
            tmp = model.name2compartments['I'].value * 0.4
            model.name2compartments['I'].value -= tmp
            model.name2compartments['Is'].value += tmp

            tmp = model.name2compartments['A'].value * 0.4
            model.name2compartments['A'].value -= tmp
            model.name2compartments['Is'].value += tmp

            tmp = model.name2compartments['P'].value * sym_ratio * 0.4
            model.name2compartments['P'].value -= tmp
            model.name2compartments['Is'].value += tmp
...
plt.title(u'上海市4.1开始分区封城+区内检测推演结果'.format('某地区', '某时间'), FontProperties=myfont)
plt.xlabel('日期')
plt.ylabel('病例数')
plt.gcf().autofmt_xdate()
# plt.savefig('{}_dailynew_confirmed_day{}.jpg'.format('香港', disp_days), dpi=200)
